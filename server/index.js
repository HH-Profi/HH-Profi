...
app.get('/api/stats', (req, res) => {
  ...
  const runRanking = db.prepare(`
    SELECT p.id, p.name, p.number,
      ROUND(SUM(r.distance_km),1) AS total_km, SUM(r.time_min) AS total_min, COUNT(*) AS run_count
    FROM runs r JOIN players p ON p.id=r.player_id WHERE r.distance_km IS NOT NULL
  ...
  res.json({ totalSessions, totalPlayers, avgAttendance, injured, sick, attendanceRanking, runRanking });
});
